...
const EnvironmentCICDVertical = ({ className = "" }) => {
    ...
    return (
        <div className={`w-1/2 bg-white rounded-lg shadow-lg ${className}`}>
            ...
            <div className="mt-4 p-4 grid grid-cols-2 gap-4">
                {ENV_LIST.map((env) => {
                    if (env.key.toLowerCase().includes("mobile")) {
                        const pipelinesListMobile = getPipelinesByEnv(
                            env.key,
                            "mobile"
                        );
                        ...
                        const latestPipelineMobile = pipelinesListMobile[0];
                        const latestReleaseMobile = releasesListMobile[0];

                        return (
                            <div
                                key={env.key}
                                className="border rounded-lg shadow-sm flex-shrink-0 bg-white hover:animate-pause"
                            >
                                <div className="p-2">
                                    <button
                                        onClick={() =>
                                    ...
                                    <div className="grid grid-cols-2 gap-2">
                                        ...
                                        <button
                                            ...
                                            className={`p-2 rounded-lg border flex items-center justify-center space-x-4
                                                            ${
                                                                latestPipelineMobile
                                                    ...
                                                    {pipelinesListMobile.map(
                                                        (pipeline) => (
                                                            <button
                                                                ...
                                                                className="w-full p-2 flex items-center justify-between bg-gray-50 rounded hover:bg-gray-100"
                                                            >
                                                                <div className="flex items-center space-x-2">
                                                                    {getStatusIcon(
                                                                        pipeline.status,
                    ...
                    const pipelinesListWeb = getPipelinesByEnv(env.key, "web");
                    const pipelinesListOthers = getPipelinesByEnv(
                        env.key,
                        "others"
                    );
                    const releasesListWeb = getReleasesByEnv(env.key, "web");
                    const releasesListOthers = getReleasesByEnv(
                        env.key,
                        "others"
                    );

                    const latestPipelineWeb = pipelinesListWeb[0];
                    const latestPipelineOthers = pipelinesListOthers[0];
                    const latestReleaseWeb = releasesListWeb[0];
                    const latestReleaseOthers = releasesListOthers[0];
                    ...
                    return (
                        ...
                        >
                            <div className="p-2">
                                <button
                                    onClick={() => toggleEnvExpansion(env.key)}
                                    className="w-full flex items-center justify-between mb-2"
                                >
                                    <h3 className="text-md font-bold">
                                        {env.value}
                                ...
                                {/* Web Section */}
                                <h3 className="font-bold text-sm my-2">Web</h3>
                                <div className="grid grid-cols-2 gap-2">
                                    {/* CI Status */}
                                    <button
                                        onClick={() =>
                                            latestPipelineWeb &&
                                            setSelectedItem(latestPipelineWeb)
                                        }
                                        className={`p-2 rounded-lg border flex items-center justify-center space-x-4
                                            ${
                                                latestPipelineWeb
                                    ...
                                    <button
                                        onClick={() =>
                                            latestReleaseWeb &&
                                            setSelectedItem(latestReleaseWeb)
                                        }
                                        className={`p-2 rounded-lg border flex items-center justify-center space-x-4
                                            ${
                                                latestReleaseWeb
                                    ...
                                    >
                                        ...
                                        {getStatusIcon(
                                            latestReleaseWeb?.environments?.[0]
                                                ?.status,
                                            latestReleaseWeb?.environments?.[0]
                                                ?.status
                                        )}
                                ...
                                {/* API Section */}
                                <h3 className="font-bold text-sm my-2">API</h3>
                                <div className="grid grid-cols-2 gap-2">
                                    {/* CI Status */}
                                    <button
                                        onClick={() =>
                                    ...
                                    <button
                                        onClick={() =>
                                            latestReleaseOthers &&
                                            setSelectedItem(latestReleaseOthers)
                                        }
                                        className={`p-2 rounded-lg border flex items-center justify-center space-x-4
                                            ${
                                                latestReleaseOthers
...
export default EnvironmentCICDVertical;
